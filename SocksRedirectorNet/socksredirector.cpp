...
struct SR_REDIRECT_RULE
{
	...
	SR_REDIRECT_RULE & operator = (const SR_REDIRECT_RULE & v)
	{
		...
		memcpy(redirectAddress, v.redirectAddress, sizeof(redirectAddress));
		redirectAddressValid = v.redirectAddressValid;
		proxies = v.proxies;
		return *this;
	}
	...
	void clear()
	{
		...
		protocol = 0;
		processId = 0;
		memset(&redirectAddress, 0, sizeof(redirectAddress));
		redirectAddressValid = false;
		proxies.clear();
	}
	...
	unsigned char	redirectAddress[NF_MAX_ADDRESS_LENGTH];
	bool		redirectAddressValid;
...
class EventHandler : public NF_EventHandler, 
	...
	public DNSBridgeProxy::DnsBridgeProxyHandler
{
	...
	DNSBridgeProxy::DnsBridgeProxy	m_dnsProxy;
...
public:
	...
	bool init()
	{
		...
		for (;;)
		{
			...
			if (!m_dnsProxy.init(this))
			{
				DbgPrint("Unable to start DNS proxy");
				break;
			}
	...
	virtual void onUdpReceiveComplete(unsigned __int64 id, char * buf, int len, char * remoteAddress, int remoteAddressLen)
	{
		...
		if (it->second->remoteAddressValid)
		{
			nf_udpPostReceive(id, it->second->remoteAddress, buf, len, it->second->m_options);
		} else
		{
	...
	virtual void onDnsBridgeReceiveComplete(unsigned __int64 id, char * buf, int len, char * remoteAddress, int remoteAddressLen)
	{
		AutoLock lock(m_cs);
		...
		DbgPrint("onDnsBridgeReceiveComplete id=%I64u len=%d remoteAddress=%s\n", id, len, remoteAddr);
#endif
	...
	virtual void tcpConnectRequest(ENDPOINT_ID id, PNF_TCP_CONN_INFO pConnInfo)
	{
...

		{
			...
			if (findTCPRule(pConnInfo, rule))
			{
				...
				{
					DbgPrint("tcpConnectRequest id=%I64u action: block\n", id);
					pConnInfo->filteringFlag = NF_BLOCK;
					return;
				}
				...
				if (!rule.proxies.empty())
				{
					if (memcmp(pAddr, rule.proxies[0].proxyAddress, addrLen) == 0)
					{
	...
	{	
...

		{
			...
			if (findUDPRule(&ci, remoteAddress, rule))
			{
				if (rule.action == ACTION_BLOCK)
				{
					DbgPrint("udpReceive id=%I64u action: block\n", id);
					return;
				}
			}
		}
	...
	virtual void udpSend(ENDPOINT_ID id, const unsigned char * remoteAddress, const char * buf, int len, PNF_UDP_OPTIONS options)
	{
		...
		char newRemoteAddress[NF_MAX_ADDRESS_LENGTH];
		bool dnsRedirect = false;

		{
			sockaddr * pAddr = (sockaddr*)remoteAddress;
			int addrLen = (pAddr->sa_family == AF_INET6)? sizeof(sockaddr_in6) : sizeof(sockaddr_in);
			memcpy(newRemoteAddress, remoteAddress, addrLen);
		}

		{
			...
			if (itid == m_udpRedirectionMap.end())
			{
				...
				if (findUDPRule(&ci, remoteAddress, rule))
				{
					...
					if (rule.action == ACTION_BLOCK)
					{
						DbgPrint("udpSend id=%I64u action: block\n", id);
						return;
					}
					...
					if (!m_udpProxy.createProxyConnection(id, rule.proxies))
					{
						nf_udpPostSend(id, remoteAddress, buf, len, options);
						return;
					}

					UDP_CONTEXT * pUdpCtx = new UDP_CONTEXT(options);

					if (rule.redirectAddressValid)
					{
						pUdpCtx->remoteAddressValid = true;
						memcpy(pUdpCtx->remoteAddress, newRemoteAddress, NF_MAX_ADDRESS_LENGTH);
						memcpy((void*)newRemoteAddress, rule.redirectAddress, NF_MAX_ADDRESS_LENGTH);
					}

					m_udpCtxMap[id] = pUdpCtx;
			...
			{
				tUdpCtxMap::iterator itcm = m_udpCtxMap.find(id);
				if (itcm != m_udpCtxMap.end())
				{
					if (itid->second.redirectAddressValid)
					{
						memcpy(itcm->second->remoteAddress, newRemoteAddress, NF_MAX_ADDRESS_LENGTH);
						itcm->second->remoteAddressValid = true;
						memcpy((void*)newRemoteAddress, itid->second.redirectAddress, NF_MAX_ADDRESS_LENGTH);
					}

					dnsRedirect = itcm->second->dnsRedirect;
				}				
			}
		}
		...
		if (dnsRedirect)
		{
			int addrLen = (((sockaddr*)newRemoteAddress)->sa_family == AF_INET)? sizeof(sockaddr_in) : sizeof(sockaddr_in6);
			if (!m_dnsProxy.udpSend(id, (char*)buf, len, (char*)newRemoteAddress, addrLen))
			{
				nf_udpPostSend(id, remoteAddress, buf, len, options);
			}
		} else
		{
			int addrLen = (((sockaddr*)newRemoteAddress)->sa_family == AF_INET)? sizeof(sockaddr_in) : sizeof(sockaddr_in6);
			if (!m_udpProxy.udpSend(id, (char*)buf, len, (char*)newRemoteAddress, addrLen))
			{
				nf_udpPostSend(id, remoteAddress, buf, len, options);
			}
		}
	...
	case OT_ACTION:
		{
			...
			if (stricmp(v.c_str(), "block") == 0)
			{
				g_currentRule.action = ACTION_BLOCK;
			} else
				return FALSE;
	...
	case OT_PROXY:
		{
			int err, addrLen;
			SR_PROXY proxy;
			std::string sAddress, sUser, sPassword;
			size_t pos;

			pos = v.find('@');
			if (pos != std::string::npos)
			{
				sAddress = v.substr(pos+1);

				std::string login = v.substr(0, pos);
				pos = login.find(':');
				if (pos != std::string::npos)
				{
					sUser = login.substr(0, pos);
					sPassword = login.substr(pos+1);
				} else
				{
					sUser = login;
				}
			...
			memset(&proxy.proxyAddress, 0, sizeof(proxy.proxyAddress));

			addrLen = sizeof(proxy.proxyAddress);
			err = WSAStringToAddress((char*)sAddress.c_str(), AF_INET, NULL, (LPSOCKADDR)&proxy.proxyAddress, &addrLen);
			if (err < 0)
			{
				addrLen = sizeof(proxy.proxyAddress);
				err = WSAStringToAddress((char*)sAddress.c_str(), AF_INET6, NULL, (LPSOCKADDR)&proxy.proxyAddress, &addrLen);
				if (err < 0)
				{
					return FALSE;
				}
			...
			proxy.userName = sUser;
			proxy.userPassword = sPassword;

			g_currentRule.proxies.push_back(proxy);

			DbgPrint("Proxy: %s", v.c_str());
		}
		break;
	...
	case OT_PROXY_ADDRESS:
		{
			...
			if (g_currentRule.proxies.empty())
				g_currentRule.proxies.push_back(SR_PROXY());

			SR_PROXY & proxy = g_currentRule.proxies[0];

			memset(&proxy.proxyAddress, 0, sizeof(proxy.proxyAddress));

			addrLen = sizeof(proxy.proxyAddress);
			err = WSAStringToAddress((char*)v.c_str(), AF_INET, NULL, (LPSOCKADDR)&proxy.proxyAddress, &addrLen);
			if (err < 0)
			{
				addrLen = sizeof(proxy.proxyAddress);
				err = WSAStringToAddress((char*)v.c_str(), AF_INET6, NULL, (LPSOCKADDR)&proxy.proxyAddress, &addrLen);
				if (err < 0)
				{
					return FALSE;
				}
