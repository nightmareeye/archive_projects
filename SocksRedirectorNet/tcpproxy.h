...
public:
	...
	bool setProxy(NFAPI_NS ENDPOINT_ID id, tProxies & proxies)
	{
		DbgPrint("TCPProxy::setProxy[%I64u]", id);
	...
	void onReceiveComplete(SOCKET socket, DWORD dwTransferred, OV_DATA * pov, int error)
	{
		...
		if (isInSocket == OUT_SOCKET)
		{
			...
			} else
			{
				...
				DbgPrint("TCPProxy::onReceiveComplete[%I64u] state %d", pov->id, proxy.proxyState);
				...
				case PS_AUTH:
					{
						...
						{
							DbgPrint("TCPProxy::onReceiveComplete[%I64u] state %d failed", pov->id, proxy.proxyState);
							break;
						}
						...
						char * realRemoteAddress;
						
						if ((pd->currentProxyIndex+1) < (int)pd->proxies.size())
						{
							realRemoteAddress = (char*)pd->proxies[pd->currentProxyIndex+1].proxyAddress;
						} else
						{
							realRemoteAddress = (char*)pd->connInfo.remoteAddress;
						}
				...
				case PS_CONNECT:
					{
						...
						if ((pd->currentProxyIndex+1) < (int)pd->proxies.size())
						{
							...
							SOCKS5_AUTH_REQUEST authReq;
							SR_PROXY & nextProxy = pd->proxies[pd->currentProxyIndex];

							authReq.version = SOCKS_5;
							authReq.nmethods = 1;

							if (!nextProxy.userName.empty())
							{
								authReq.methods[0] = S5AM_UNPW;
							} else
							{
...

		{
			...
			if (getRemoteAddress(pRemoteAddr, &connInfo))
			{
				...
				if (!pd->proxies.empty())
				{
					...
					pAddr = (sockaddr*)pd->proxies[0].proxyAddress;
					dwLen = sizeof(remoteAddr);
					...
					memcpy(realRemoteAddress, pd->proxies[0].proxyAddress, sizeof(realRemoteAddress));
				} else
				{
