...
public:
	...
	bool createProxyConnection(unsigned __int64 id, tProxies & proxies)
	{
		bool result = false;
		...
		if (proxies.empty())
		{
			DbgPrint("UDPProxy::createProxyConnection() no proxy");
			return false;
		}

		{
			...
			pAddr = (sockaddr*)proxies[0].proxyAddress;
			...
			DbgPrint("UDPProxy::createProxyConnection() redirect to SOCKS proxy %s", remoteAddr);
#endif

			memcpy(realRemoteAddress, proxies[0].proxyAddress, sizeof(realRemoteAddress));
		}
		...
		for (;;)
		{
			...
			if (!startConnect(tcpSocket, 
				...
				(((sockaddr*)realRemoteAddress)->sa_family == AF_INET)? 
...
//		DbgPrint("UDPProxy::onUdpSendComplete %I64u bytes=%d, err=%d", pov->id, dwTransferred, error);
	...
	void onConnectComplete(SOCKET socket, DWORD dwTransferred, OV_DATA * pov, int error)
	{
...

		{
			...
			if (it != m_socketMap.end())
			{
				SR_PROXY & proxy = it->second->proxies[0];
...
//		DbgPrint("UDPProxy::onTcpSendComplete %I64u bytes=%d, err=%d", pov->id, dwTransferred, error);
	...
	bool startProxyConnect(PROXY_DATA * pd, unsigned __int64 id)
	{
		if ((pd->currentProxyIndex + 1) >= (int)pd->proxies.size())
		{
			return false;
		}

		pd->currentProxyIndex++;

		SR_PROXY & proxy = pd->proxies[pd->currentProxyIndex];

		char * proxyRemoteAddress;
		proxyRemoteAddress = (char*)proxy.proxyAddress;

		int ipFamily = ((sockaddr*)proxyRemoteAddress)->sa_family;

		int proxyRemoteAddressLen = (ipFamily == AF_INET)? sizeof(sockaddr_in) : sizeof(sockaddr_in6);
		...
		if (ipFamily == AF_INET)
		{
			...
			req.address = ((sockaddr_in*)proxyRemoteAddress)->sin_addr.S_un.S_addr;
			req.port = ((sockaddr_in*)proxyRemoteAddress)->sin_port;

			if (startTcpSend(pd->tcpSocket, (char*)&req, sizeof(req), id))
			{
				proxy.proxyState = PS_CONNECT;
			}		
		} else
		{
			...
			memcpy(&req.address, &((sockaddr_in6*)proxyRemoteAddress)->sin6_addr, 16);
			req.port = ((sockaddr_in6*)proxyRemoteAddress)->sin6_port;
	...
	void onTcpReceiveComplete(SOCKET socket, DWORD dwTransferred, OV_DATA * pov, int error)
	{
...

		{
			...
			if (it != m_socketMap.end())
			{
				SR_PROXY & proxy = it->second->proxies[it->second->currentProxyIndex];

				DbgPrint("UDPProxy::onTcpReceiveComplete %I64u state=%d", pov->id, proxy.proxyState);
				...
				case PS_CONNECT:
					{
						...
						{
							DbgPrint("UDPProxy::onTcpReceiveComplete %I64u failed on state %d", pov->id, proxy.proxyState);
							break;
						}
				...
				case PS_AUTH:
					{
						...
						if (pr->method == S5AM_UNPW && !proxy.userName.empty())
						{
							std::vector<char> authReq;

							authReq.push_back(1);
							authReq.push_back((char)proxy.userName.length());
							authReq.insert(authReq.end(), proxy.userName.begin(), proxy.userName.end());
							authReq.push_back((char)proxy.userPassword.length());
							
							if (!proxy.userPassword.empty())
								authReq.insert(authReq.end(), proxy.userPassword.begin(), proxy.userPassword.end());

							if (startTcpSend(it->second->tcpSocket, (char*)&authReq[0], (int)authReq.size(), pov->id))
							{
								proxy.proxyState = PS_AUTH_NEGOTIATION;
							}

							break;
						}
/*
						if ((it->second->currentProxyIndex + 1) < (int)it->second->proxies.size())
						{
							startProxyConnect(it->second, pov->id);
							break;
						}
*/
						SOCKS5_REQUEST_IPv4 req;
				...
				case PS_UDP_ASSOC:
					{
						...
						if (pr->address_type == SOCKS5_ADDR_IPV4)
						{
							...
							memset(&addr, 0, sizeof(addr));
							addr.sin_family = AF_INET;
							addr.sin_addr.S_un.S_addr = ((sockaddr_in*)proxy.proxyAddress)->sin_addr.S_un.S_addr;
						...
						if (pr->address_type == SOCKS5_ADDR_IPV6)
						{
							...
							memset(&addr, 0, sizeof(addr));
							addr.sin6_family = AF_INET6;
							memcpy(&addr.sin6_addr, &((sockaddr_in6*)proxy.proxyAddress)->sin6_addr, 16);
